...
const create = async (newUserFine) => {
    try {
        ...
        const [rows, fields] = await connection.query(`INSERT INTO user_fine (user_id, fine_id, start_date_time) VALUES ?`, [
...
const deleteByUserIdAndFineId = async (userId, fineId) => {
    try {
        const connection = await connectionPromise;
        const [rows, fields] = await connection.query(`DELETE FROM author WHERE user_id = ${userId} AND fine_id = ${fineId};`);
...
const getByUserIdAndFineId = async (userId, fineId) => {
    try {
        const connection = await connectionPromise;
        const [rows, fields] = await connection.query(`SELECT * FROM user_fine WHERE user_id = ${userId} AND fine_id = ${fineId};`);
