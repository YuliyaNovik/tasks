...
CREATE TABLE IF NOT EXISTS user(id int AUTO_INCREMENT NOT NULL, email NVARCHAR(128) UNIQUE, role_id int NOT NULL, first_name NVARCHAR(256), last_name NVARCHAR(256), address NVARCHAR(256), password_hash VARCHAR(512), PRIMARY KEY(id), FOREIGN KEY (role_id) REFERENCES role(id) ON DELETE CASCADE ON UPDATE CASCADE);
